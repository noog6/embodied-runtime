...
* `inspect_job_runtime({})` returns only the current Job occurrence. An idle runtime
  returns `status=idle`. Active output keeps Job, JobRun, Task, and goal identity;
...
where useful, and the current run ID. Invalid fields or bounds receive a structured
